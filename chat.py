...
class Chat():
	...
	class ListenThread(threading.Thread):
		...
		def run(self):
			...
			while True:
				# try to get message and output it
				try:
					...
					#print(new_msg)
				# Catch a JSON encoding error (empty message the first time); do nothing
	...
	def print_disp(self, window, content, prefix=""):
		...
		for line in lines:
			...
			# move cursor to bottom of display window
			window.move(Y-2,1)
			# clear line of excess text
			window.clrtoeol()
			# place new content here
			window.addstr(Y-2,1,prefix+" "+line)
			# redraw border for display window
			window.border()
	...
	def main(self, argv):
		...
		try:
			opts, args = getopt.getopt(argv,"c:hn:p:q:r:s:t:",["clients=","name=","lhost=","lport=","rhost=","rport=","type="])
		...
		if (self.PROTOCOL == "tcp"):
			...
			try:
				...
				if (self.REMOTEHOST == ""):
					...
					self.hostsocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # create a socket object
					...
					self.hostsocket.bind( (self.LOCALHOST,self.LISTENPORT) )							 # bind to the port
					...
					self.hostsocket.listen(self.PEERS)
				# if the peer (initial connection attempt to remote peer),
					...
					self.peersocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)	# create a socket object
					self.print_disp(disp_win,"Connecting to peer "+str(self.REMOTEHOST)+" with port "+str(self.SENDPORT)+"...",self.INFO)
					self.peersocket.connect( (self.REMOTEHOST,self.SENDPORT) )								 # actively initiate TCP connection with host and port
				...
				print("Error in connecting:"+str(err))
			...
			while True:
				...
				if (self.REMOTEHOST == ""):
					...
					self.peersocket, addr = self.hostsocket.accept()
					self.print_disp(disp_win, "Connection established to "+str(addr),self.INFO)
					# send message to client
					msg = "Connection to server established."
					self.peersocket.send(msg.encode('ascii'))
					# receive name from client as string, converted to JSON
					init_msg_json = json.loads(self.peersocket.recv(1024).decode('ascii'))
					# get name of sender
					new_name = init_msg_json['name']
					# get initial message of sender
					new_msg  = init_msg_json['message']
					# print connection to new peer message
					self.print_disp(disp_win,"New peer connected: "+str(new_name),self.INFO)
					# add new peer to list of peers
					self.peer_names.append(new_name)
					# print new peer's initial message
					self.print_disp(disp_win,new_msg,"["+new_name+"]")
				#if the peer,
				# else:
			...
			send_thread = self.SendThread(1, "Send-Thread", self.peersocket,input_win, disp_win, self.NAME)
			listen_thread = self.ListenThread(2, "Listen-Thread", self.peersocket,input_win, disp_win)
			# start threads
			send_thread.start()
			listen_thread.start()
